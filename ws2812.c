...
int main() {
    ...
    PIO pio = pio0;//3
    int sm = 0;//4
    uint offset = pio_add_program(pio, &ws2812_program);//5,6

    ws2812_program_init(pio, sm, offset, WS2812_PIN, 800000, IS_RGBW);//7

    int t = 0;//30
    while (1) {//31
        int pat = rand() % count_of(pattern_table);//32,33,34
        int dir = (rand() >> 30) & 1 ? 1 : -1;//35,36,37,38,39
        puts(pattern_table[pat].name);//40
        puts(dir == 1 ? "(forward)" : "(backward)");//41,42,43
        for (int i = 0; i < 1000; ++i) {//44
            pattern_table[pat].pat(NUM_PIXELS, t);//45
            sleep_ms(10);
            t += dir;
        }
    }
